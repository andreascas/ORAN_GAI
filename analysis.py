...
def get_end_to_end_latency(ue1_file,ue2_file):
    ...
    for key in list(ue2.keys())[10:]:
        if key in ue1.keys():
            ue2_t.append(ue2[key])
            ue1_t.append(ue1[key])
    e2e = (np.array(ue2_t)-np.array(ue1_t))/1000000
    return e2e, ue1_t, ue2_t


experiment_folders = [
                      'olddata/newincreased/newincreased_tradscale/',
                      'olddata/oldnewincreased/newincreased_gaiscale/',
                      'olddata/newincreased/newincreased_noscale/'] 
experiment_folders = ['olddata/big/big_tradscale/',
                      'olddata/big/big_gaiscale/',
                      'olddata/big/big_noscale/'
                      ] 

experiment_folders = ["globecomdata/ABR/",
    "globecomdata/GAI_BASED1/"
                      ]
